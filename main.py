...
def get_branches(request: BranchRequest):
    ...
    branches = gh.get_branches(owner, repo)
    if not branches:
         # It might be empty or failed.
         # For better UX we could check if it failed or just empty.
         pass
    return branches

@app.get("/api/branches")
def get_branches_get(url: str):
    """
    Fetches available branches via GET for easier usage.
    """
    try:
    ...
    gh = GithubService()
    return gh.get_branches(owner, repo)
...
def api_ingest(request: IngestRequest):
    ...
    try:
        ...
        # Parse Repo ID from output
        repo_id = f"{owner}-{repo}-{request.branch}" # Fallback
        for line in result.stdout.splitlines():
            if "Repo ID:" in line:
                repo_id = line.split("Repo ID:")[1].strip()
